...
const render = () => {
  ...
  ctx.drawImage(engineImg, state.player.x-2, state.player.y + 15, state.player.w, state.player.h);
  // Bullets
...
playBtn.addEventListener("click", () => {
  ...
  state.score = 0;
  state.bullets = [];
  state.enemies = (function spawn(){ const cols = 9, rows = 3; return Array.from({length: cols*rows}, (_, i) => ({ x: 40 + (i%cols) * ((canvas.width-80)/cols), y: 40 + Math.floor(i/cols)*40, w: 36, h: 18, alive: true })); })();
  requestAnimationFrame(loop);
});
